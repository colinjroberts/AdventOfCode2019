...
class Point:
    ...
    def __str__(self):
        return "({0},{1}):{2}".format(self.x, self.y, self.steps)
...
def main():
    ...
    # Create direction set and starting point
    pset1 = set()
    pset2 = set()
    currentPoint1 = Point(0,0,0)
    currentPoint2 = Point(0,0,0)
    ...
    for direction in direction2:
        ...
        for step in range(int(direction[1:])):
            ...
            if (direction[0] == "U"):
                # Add steps to the top x times
                direction2Q.put(Point(0, 1, 1))

            if (direction[0] == "D"):
                # Add steps to the bottom x times
                direction2Q.put(Point(0, -1, 1))

    print("Finished interpreting directions")

    print("About to start processing instructions")
    # For each instruction
    while not direction1Q.empty():
        currentInstruction = direction1Q.get()
        currentPoint1 = currentPoint1.step(currentInstruction)
        #print(currentPoint)
        if(not currentPoint1 in pset1):
            pset1.add(Point(currentPoint1.x, currentPoint1.y, currentPoint1.steps))

    while not direction2Q.empty():
        currentInstruction = direction2Q.get()
        currentPoint2 = currentPoint2.step(currentInstruction)
        #print(currentPoint)
        if(not currentPoint2 in pset2):
            pset2.add(Point(currentPoint2.x, currentPoint2.y, currentPoint2.steps))
    ...
    for item in pset1:
        #print(item)
        pDict1[item] = item.steps

    pDict2 = {}
    for item in pset2:
        pDict2[item] = item.steps

    minLength = 10000000000

    for item in psetIntersect:
        l1 = pDict1.get(item)
        l2 = pDict2.get(item)
