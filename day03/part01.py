class Point:
  ...
  def pointsBtwn(self, other):
    points = set({})
    if(self.x != other.x):
      points.add(Point(self.x, self.y))
      for xval in range(abs(self.x-other.x)):
        if(self.x>other.x):
          points.add(Point(self.x-(xval+1), self.y))
          #print(self.x-(xval+1), self.y)
        else:
          points.add(Point(self.x+xval+1, self.y))
          #print(self.x+xval+1, self.y)
      ...
      points.add(Point(self.x, self.y))
      for yval in range(abs(self.y-other.y)):
        if(self.y>other.y):
          points.add(Point(self.x, self.y-(yval+1)))
          #print(self.x, self.y-(yval+1))
        else:
          points.add(Point(self.x, self.y+(yval+1)))
          #print(self.x, self.y+(yval+1))
...
def main():
  ...
  instructions1 = input1.split(",")
  instructions2 = input2.split(",")
  ...
  for instruction in instructions1:
    origin1 += instruction
    wire1.append(origin1)

  origin2 = Point(0,0)
  for instruction in instructions2:
    origin2 += instruction
    wire2.append(origin2)

  #print("wire1:", wire1)
  #print("wire2:", wire2)

  wire1Path = set()
  for n in range(len(wire1)-1):
    #print(wire1[n], wire1[n+1])
    paths = set(wire1[n].pointsBtwn(wire1[n+1]))
    wire1Path = wire1Path.union(paths)

  wire2Path = set()
  for n in range(len(wire2)-1):
    #print(wire2[n], wire2[n+1])
    paths = set(wire2[n].pointsBtwn(wire2[n+1]))
    wire2Path = wire2Path.union(paths)
  
  #print("wire1 path:", wire1Path)
  #print("wire1 path:", wire2Path)

  overlaps = wire1Path & wire2Path
  overlaps = overlaps - set({Point(0,0)})
  print("Wire Intersections:", overlaps)

  manDist = set({})
  for point in overlaps:
    manDist.add(abs(point.x) + abs(point.y))
  print(min(manDist))
